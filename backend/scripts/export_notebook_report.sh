#!/bin/sh
# Export notebook to HTML and optionally PDF.
...
NOTEBOOK="${1:-notebooks/polymarket_deep_insights.ipynb}"
...
if [ ! -f "$NOTEBOOK" ]; then
  echo "Missing notebook: $NOTEBOOK"
  exit 1
fi
...
if uv run python -c "import playwright" 2>/dev/null; then
  ...
  echo "Done: $OUT_DIR/$(basename "${NOTEBOOK%.ipynb}").pdf"
else
  echo ""
  ...
  echo "  2) Abre $OUT_DIR/$(basename "${NOTEBOOK%.ipynb}").html en el navegador → Imprimir → Guardar como PDF"
fi

echo "HTML: $OUT_DIR/$(basename "${NOTEBOOK%.ipynb}").html"
